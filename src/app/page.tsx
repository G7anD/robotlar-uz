...
export default async function Home() {
  const { categories, featuredRobots, latestNews } = await getData();
    ...
    <div>
      ...
      <section className="relative overflow-hidden grid-bg">
        <div
          ...
          style={{ background: "radial-gradient(ellipse 80% 60% at 50% -10%, rgba(0,212,255,0.15) 0%, transparent 60%)" }}
        />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-36 relative">
          <div className="text-center max-w-3xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/yangiliklar" className="px-8 py-3 rounded-full font-semibold text-sm transition-all hover:opacity-90 hover:-translate-y-0.5" style={{ background: "linear-gradient(135deg, #00d4ff, #7c3aed)", color: "#fff" }}>
                Yangiliklar →
              </Link>
              <Link href="/robotlar" className="px-8 py-3 rounded-full font-semibold text-sm border transition-all hover:border-cyan-400" style={{ borderColor: "var(--border)", color: "var(--foreground)" }}>
                Robot Katalogi
              </Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6 max-w-2xl mx-auto mt-20">
            {[{ value: "500+", label: "Robot Profili" }, { value: "50+", label: "Kategoriya" }, { value: "Har Kuni", label: "Yangiliklar" }].map((stat) => (
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {categories.map((cat) => (
            <Link key={cat.slug} href={`/robotlar/${cat.slug}`} className="card-glow rounded-xl p-5 text-center transition-all hover:-translate-y-1 border" style={{ background: "var(--card-bg)", borderColor: "var(--border)" }}>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {featuredRobots.slice(0, 6).map((robot) => (
            <Link key={robot.id} href={`/robotlar/${robot.categorySlug}/${robot.slug}`} className="card-glow rounded-2xl p-6 border transition-all hover:-translate-y-1" style={{ background: "var(--card-bg)", borderColor: "var(--border)" }}>
              <div className="flex items-start justify-between mb-4">
                <div className="w-14 h-14 rounded-xl flex items-center justify-center text-3xl" style={{ background: "rgba(0,212,255,0.08)" }}>
                  {robot.emoji}
                </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {latestNews.map((article) => (
            <Link key={article.id} href={`/yangiliklar/${article.slug}`} className="card-glow rounded-2xl border overflow-hidden transition-all hover:-translate-y-1" style={{ background: "var(--card-bg)", borderColor: "var(--border)" }}>
      ...
      {/* Telegram CTA */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="rounded-3xl p-10 md:p-16 text-center relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(0,212,255,0.08) 0%, rgba(124,58,237,0.12) 100%)", border: "1px solid rgba(0,212,255,0.15)" }}>
          ...
          <a href="https://t.me/robotlaruz" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-8 py-3 rounded-full font-semibold text-sm transition-all hover:opacity-90" style={{ background: "linear-gradient(135deg, #00d4ff, #7c3aed)", color: "#fff" }}>
