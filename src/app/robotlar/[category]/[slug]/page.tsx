...
import { featuredRobots, categories } from "@/lib/data";
...
export async function generateStaticParams() {
  return featuredRobots.map((r) => ({ category: r.categorySlug, slug: r.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const robot = featuredRobots.find((r) => r.slug === slug);
  if (!robot) return {};
  return {
...
export default async function RobotDetailPage({ params }: Props) {
  ...
  const robot = featuredRobots.find((r) => r.slug === slug && r.categorySlug === category);
  if (!robot) notFound();

  const cat = categories.find((c) => c.slug === category);
  const related = featuredRobots.filter((r) => r.categorySlug === category && r.id !== robot.id).slice(0, 3);

  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <Link
        href={`/robotlar/${category}`}
      ...
      <div className="grid md:grid-cols-2 gap-10 mb-12">
        <div
          className="rounded-3xl flex items-center justify-center h-64 md:h-auto text-9xl border"
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {related.map((r) => (
              <Link
                key={r.id}
                href={`/robotlar/${r.categorySlug}/${r.slug}`}
                className="card-glow rounded-xl p-5 border transition-all hover:-translate-y-1"
                style={{ background: "var(--card-bg)", borderColor: "var(--border)" }}
              >
